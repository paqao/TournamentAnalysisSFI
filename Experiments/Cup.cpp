...
namespace Experiments {
	...
	int sortPlayers(const PlayerResult* arg1, const PlayerResult* arg2)
	{
	...
	Fixtures::IFixtureGenerator* Cup::GetFixtureGenerator(int index) {
		return this->fixtureGenerator;
	}

	Fixtures::IFixtureGenerator* Cup::GetFixtureGenerator(int index, int round, Tournaments::Model::Tournament* data) {
		return this->fixtureGenerator;
	}

	Tournaments::Model::Tournament* Cup::PrepareTestData(int index) {
		auto data = tournamentGenerator->Generate();
		return data;
	}

	MatchResolvers::IMatchResolver* Cup::GetMatchResolver(int index) {
		return new MatchResolvers::StandardMatchResolver();
	}

	void Cup::CalculateResults(Tournaments::Model::Tournament* data) {
		...
		qsort(data->results, playersCount, sizeof(PlayerResult), (int(*)(const void*, const void*)) sortPlayers);
