...
namespace Experiments {
	...
	class Cup : public IExperiment {
	private:
		Fixtures::IFixtureGenerator* fixtureGenerator;

	public:
		Cup(int totalPlayers, int countLimit) : IExperiment() {
			//this->metrics->push_back(new Metrics::MatchCountProcessor());
			ExperimentName = "cup_from_" + std::to_string(totalPlayers) + "_limit_" + std::to_string(countLimit);
