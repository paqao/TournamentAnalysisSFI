...
namespace Experiments {
	...
	int swissSortPlayers(const PlayerResult* arg1, const PlayerResult* arg2)
	{
	...
	Fixtures::IFixtureGenerator* Swiss::GetFixtureGenerator(int index) {
		return this->generator;
	}

	Tournaments::Model::Tournament* Swiss::PrepareTestData(int index) {
		auto data = tournamentGenerator->Generate();
		return data;
	}

	MatchResolvers::IMatchResolver* Swiss::GetMatchResolver(int index) {
		return new MatchResolvers::StandardMatchResolver();
	}

	void Swiss::CalculateResults(Tournaments::Model::Tournament* data) {
		...
		qsort(data->results, playersCount, sizeof(PlayerResult), (int(*)(const void*, const void*)) swissSortPlayers);
	...
	Fixtures::IFixtureGenerator* Swiss::GetFixtureGenerator(int index, int round, Tournaments::Model::Tournament* data) {
		if (round == 0) {
			return this->firstRound;
		};
		if (round == data->GetRounds().size() - 1) {
