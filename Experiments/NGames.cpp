...
namespace Experiments {
	...
	int ngamesSortPlayers(const PlayerResult* arg1, const PlayerResult* arg2)
	{
	...
	Fixtures::IFixtureGenerator* NGames::GetFixtureGenerator(int index) {
		return this->generator;
	}

	Tournaments::Model::Tournament* NGames::PrepareTestData(int index) {
		auto data = tournamentGenerator->Generate();
		return data;
	}

	MatchResolvers::IMatchResolver* NGames::GetMatchResolver(int index) {
		return new MatchResolvers::StandardMatchResolver();
	}

	void NGames::CalculateResults(Tournaments::Model::Tournament* data) {
		...
		qsort(data->results, playersCount, sizeof(PlayerResult), (int(*)(const void*, const void*)) ngamesSortPlayers);
	...
	Fixtures::IFixtureGenerator* NGames::GetFixtureGenerator(int index, int round, Tournaments::Model::Tournament* data) {
		return this->generator;
	}
}